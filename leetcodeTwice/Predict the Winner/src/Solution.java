...
public class Solution {
	...
	// 第一个参与者
	public boolean first(int s1, int s2, int[] nums, int start, int end) {
		if (start >= end) {
			if (s1 >= s2) {
				return true;
			} else
		...
		// 如果second返回true，那就说明是第二个参赛者赢，第一个参赛者输，所以取反
		return !second(s1 + nums[start], s2, nums, start + 1, end) || !second(s1 + nums[end], s2, nums, start, end - 1);
	}

	// 第二个参与者
	public boolean second(int s1, int s2, int[] nums, int start, int end) {
		if (start >= end) {
			if (s1 < s2) {
				return true;
			} else
		...
		// 如果first返回true，那就说明是第一个参赛者赢，第二个参赛者输，所以取反
