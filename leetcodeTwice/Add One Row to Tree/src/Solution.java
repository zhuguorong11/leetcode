...
public class Solution {
	...
	public void addRow(TreeNode node, int v, int d, int curDepth) {
		if (node == null)
			return;
		if (curDepth == d - 1) {
			// left
			TreeNode tempLeft = node.left;
			TreeNode inserLeft = new TreeNode(v);
			node.left = inserLeft;
			inserLeft.left = tempLeft;

			// right
			TreeNode tempRight = node.right;
			TreeNode inserRight = new TreeNode(v);
			node.right = inserRight;
			inserRight.right = tempRight;
		}
